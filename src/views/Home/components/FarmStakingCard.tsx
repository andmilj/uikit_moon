...
import useVsFarmsWithBalance from 'hooks/useVsFarmsWithBalance'
...
const FarmedStakingCard = () => {
  ...
  const vsFarmsWithBalance = useVsFarmsWithBalance()

  const cakeBalance = getBalanceNumber(useTokenBalance(getCakeAddress()))
  const eggPrice = useCakePrice().toNumber()
  const allEarnings = useAllEarnings()
  const earningsSum = allEarnings.reduce((accum, earning) => {
    return accum + new BigNumber(earning).div(new BigNumber(10).pow(18)).toNumber()
  }, 0)
  const balancesWithValue = farmsWithBalance
    .filter((balanceType) => balanceType.balance.toNumber() > 0)
    .map((f) => f.pid)
  const vsBalancesWithValue = vsFarmsWithBalance
    .filter((balanceType) => balanceType.balance.toNumber() > 0)
